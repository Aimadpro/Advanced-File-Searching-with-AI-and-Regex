...
class SmartSearch:
    ...
    def ia_search(self, prompt, model_name='gpt-3.5-turbo-0125', max_tokens=100):
        ...
        for file, text in self.files.items():
            ...
            if not api_key:
                raise ValueError("La clave API de OpenAI no está configurada en las variables de entorno.")
...
def cargar_parser(sub_parser="parser"):
    load_dotenv()
    sub_parser = argparse.ArgumentParser(description="Herramienta para realizar búsquedas en ficheros utilizando expresiones regulares o modelos de IA.")
    sub_parser.add_argument("file", type=str, help="Ruta del fichero en el que realizar la búsqueda.")
    sub_parser.add_argument("-p", "--prompt", type=str, help="Prompt para la búsqueda con IA.")
    sub_parser.add_argument("-r", "--regex", type=str, help="Expresión regular para la búsqueda.")
    sub_parser.add_argument("-m", "--model", type=str, default="gpt-3.5-turbo-0125", help="Modelo de IA a utilizar.")
    sub_parser.add_argument("--max-tokens", type=int, default=100, help="Número máximo de tokens que el modelo puede generar.")
    args = sub_parser.parse_args()

    return args

def ejecutar_smartsearch(args):
